...
class DatacenterRepository:
    ...
    def add(self, datacenter: Datacenter):
        self.session.add(datacenter)

    def get(self, datacenter_id: int) -> Optional[Datacenter]:
        return self.session.query(Datacenter).filter_by(id=datacenter_id).first()

    def get_by_name(self, name: str) -> Optional[Datacenter]:
        return self.session.query(Datacenter).filter_by(name=name).first()

    def list(self) -> List[Datacenter]:
        return self.session.query(Datacenter).all()

    def delete(self, datacenter: Datacenter):
        self.session.delete(datacenter)
...
class CoreRepository:
    ...
    def list(self, datacenter_id: Optional[int] = None) -> List[Core]:
        query = self.session.query(Core)
        if datacenter_id:
            query = query.filter_by(datacenter_id=datacenter_id)
        return query.all()
...
class VlanRepository:
    ...
    def list(self, core: Optional[Core] = None) -> List[Vlan]:
...
class VlanRestrictionRangeRepository:
    ...
    def list(self, core: Optional[Core] = None) -> List[VlanRestrictionRange]:
